...
export async function getProductsDB(): Promise<Product[] | null> {
	...
	const { rows }: QueryResult<Product> = await con.query(
		"SELECT p.product_id,p.name,p.description,p.image,p.price,r.name as rarity,r.rarity_id,p.created_at,p.updated_at FROM products p INNER JOIN rarities r ON p.rarity = r.rarity_id"
	);
